...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(translateAuthError(err?.message, 'فشل إنشاء الحساب'))
    } finally {
      setLoading(false)
    }
  }
  ...
  if (success) {
    return (
      ...
      >
        <div className="w-full max-w-md bg-slate-800/80 rounded-2xl border border-slate-700 p-8 shadow-xl text-center">
          <Gift className="text-amber-400 mx-auto mb-4" size={40} />
          <h1 className="text-xl font-bold text-white mb-2">تحقق من بريدك</h1>
          <p className="text-slate-400 text-sm mb-6">
            تم إرسال رابط التأكيد إلى <strong className="text-slate-300">{email}</strong>. افتح الرابط لتفعيل الحساب ثم سجّل الدخول.
          </p>
          <Link
            to="/login"
            className="inline-block py-2.5 px-4 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold transition-colors"
          >
            تسجيل الدخول
          </Link>
        </div>
    ...
    >
      <div className="w-full max-w-md bg-slate-800/80 rounded-2xl border border-slate-700 p-8 shadow-xl">
        ...
        <p className="text-slate-400 text-center text-sm mb-4">
          عجلة الحظ لمتاجر سلة. أنشئ حساباً بالبريد الإلكتروني وكلمة مرور قوية.
        </p>
