...
export default function DashboardPanel(props) {
  const {
    ...
    tempLogo, setTempLogo, onLogoUpload, tempWheelStyle, setTempWheelStyle,
  ...
  const planInfo = currentPlan != null ? getPlanInfo(currentPlan) : getPlanInfo('free');
                      ...
                      <div className="flex flex-col h-full">
                          ...
                          <div className="flex-1 overflow-y-auto p-6 bg-slate-100 relative">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                  {/* --- إعدادات الشعار --- */}
                                  <div 
                                    className={`bg-white p-6 rounded-xl shadow-sm border border-slate-200 relative ${!planInfo.canCustomLogo ? 'cursor-pointer' : ''}`}
                                    onClick={!planInfo.canCustomLogo ? showUpgradeToast : undefined}
                                  >
                                  ...
                                  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 md:col-span-2">
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                          ...
                                          <button
                                              ...
                                              className="p-4 rounded-xl border-2 border-slate-200 bg-slate-50 cursor-pointer hover:bg-amber-50 hover:border-amber-300 transition-all opacity-80"
                                          >
                                              <div className="text-center">
                                                  <div className="w-24 h-24 mx-auto mb-2 relative">
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 467 501" fill="none" className="w-full h-full opacity-70">
                                                          <g filter="url(#filter0_i_27_286)">
                                                              <path d="M464.63 265.863C465.362 393.747 362.285 498.011 234.401 498.743C106.517 499.475 2.25341 396.398 1.52151 268.514C0.789611 140.63 103.867 36.3664 231.751 35.6345C359.634 34.9026 463.898 137.98 464.63 265.863ZM25.1857 268.379C25.8428 383.193 119.451 475.736 234.266 475.079C349.08 474.422 441.623 380.813 440.966 265.999C440.309 151.184 346.7 58.6416 231.886 59.2987C117.071 59.9558 24.5286 153.564 25.1857 268.379Z" fill="#2BD0EE"/>
                                                          </g>
                                                          <g filter="url(#filter1_d_27_286)">
                              ...
                              {/* --- إعدادات الخلفية --- */}
                              <div 
                                className={`bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 relative ${!planInfo.canCustomBackground ? 'cursor-pointer' : ''}`}
                                onClick={!planInfo.canCustomBackground ? showUpgradeToast : undefined}
                              >
                              ...
                              {/* --- إعدادات الأصوات --- */}
                              <div 
                                className={`bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 relative ${!planInfo.canCustomSounds ? 'cursor-pointer' : ''}`}
                                onClick={!planInfo.canCustomSounds ? showUpgradeToast : undefined}
                              >
                              ...
                              <div 
                                className={`bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 relative ${!planInfo.canSocialLinks ? 'cursor-pointer' : ''}`}
                                onClick={!planInfo.canSocialLinks ? showUpgradeToast : undefined}
                              >
                              ...
                              {/* --- إعدادات الفوتر --- */}
                              <div 
                                className={`bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 relative ${!planInfo.canFooterSettings ? 'cursor-pointer' : ''}`}
                                onClick={!planInfo.canFooterSettings ? showUpgradeToast : undefined}
                              >
